...
# استخدام المفتاح من config
supabase: Client = create_client(config.SUPABASE_URL, config.SUPABASE_KEY)

def init_db():
    """إنشاء الجداول تلقائياً باستخدام Service Role Key."""
    try:
        ...
        sqls = [
            "CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY, points INTEGER DEFAULT 1, invited_by BIGINT DEFAULT NULL, invite_count INTEGER DEFAULT 0);",
            "CREATE TABLE IF NOT EXISTS banned_users (user_id BIGINT PRIMARY KEY, banned_at TIMESTAMP DEFAULT NOW());",
            "CREATE TABLE IF NOT EXISTS gift_links (code TEXT PRIMARY KEY, points INTEGER NOT NULL, max_uses INTEGER NOT NULL, used_count INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT NOW());",
...
def db_add_user(user_id, invited_by=None):
    try:
        ...
        if invited_by and invited_by != user_id:
            supabase.table('users').insert({'user_id': user_id, 'points': 1, 'invited_by': invited_by}).execute()
            supabase.table('users').update({
                'points': supabase.raw('points + 1'),
                'invite_count': supabase.raw('invite_count + 1')
            }).eq('user_id', invited_by).execute()
        else:
            supabase.table('users').insert({'user_id': user_id, 'points': 1}).execute()
        return True
    except Exception as e:
...
def db_add_points(user_id, amount):
    try:
        supabase.table('users').update({'points': supabase.raw(f'points + {amount}')}).eq('user_id', user_id).execute()
        return True
    except Exception as e:
...
def db_create_gift(points, max_uses):
    ...
    try:
        supabase.table('gift_links').insert({'code': code, 'points': points, 'max_uses': max_uses}).execute()
        return code
    except:
        return None
...
def db_use_gift(code):
    try:
        ...
        if gift['used_count'] >= gift['max_uses']:
            return 'expired'
        supabase.table('gift_links').update({'used_count': supabase.raw('used_count + 1')}).eq('code', code).execute()
        return 'success'
    except:
        return None
