...
from telethon.tl.types import User, ChannelParticipantsAdmins, InputPhoneContact
...
# باقي الدوال (get_filtered_members, is_valid_member, add_to_contacts, add_to_target)
...
async def get_filtered_members(source, count, user_id):
    try:
        ...
        for member in all_members:
            ...
            if await is_valid_member(member, target_user_ids, entity):
...
async def is_valid_member(member, target_user_ids, source_entity):
    if member.bot:
        return False
    ...
    try:
        if hasattr(member, 'participant') and hasattr(member.participant, 'admin_rights'):
            return False
    except:
        pass
...
async def add_to_contacts(members, user_id):
    ...
    for i, member in enumerate(members):
        ...
        if isinstance(member, User) and member.phone:
